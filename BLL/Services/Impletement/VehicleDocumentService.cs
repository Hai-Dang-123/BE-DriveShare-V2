    ...
    public class VehicleDocumentService : IVehicleDocumentService
    {
        ...
        public async Task<ResponseDTO> AddDocumentAsync(Guid vehicleId, AddVehicleDocumentDTO dto)
        {
            try
            {
                ...
                return new ResponseDTO("Đã tải lên giấy tờ. Vui lòng chờ nhân viên duyệt.", 201, true, document);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ResponseDTO> ReviewVehicleDocumentAsync(Guid documentId, bool isApproved, string? rejectReason)
        {
            try
            {
                // (Controller cần check Role STAFF/ADMIN)
                ...
                if (isApproved)
                {
                    ...
                    doc.ProcessedAt = DateTime.UtcNow; // Nếu có trường này
                ...
                return new ResponseDTO(isApproved ? "Đã duyệt giấy tờ xe." : "Đã từ chối giấy tờ xe.", 200, true);
            }
            catch (Exception ex)
            {
